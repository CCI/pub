...
choices that end up emphasizing some of the more desirable metrics
...
draw upon years of diverse networking research spanning from theories
of developer best practices to systems-level implementation
algorithms.  The goal was to create a relatively simple, high-level
communications API -- one that might even feel somewhat ``familiar''
to developers because of its roots in many other, existing network
APIs -- that still provides important features such as scalability,
resiliency, and performance.  The result is the Common Communications
Infrastructure (CCI) API: an API that is simple enough to implement in
hardware, explicitly designed to incur minimal overhead from a host
operating system, and performant enough to meet the needs of
