...
increasing prices and the risks of business failure or market disruptions. It 
...
Simplicity is paramount to the success of a programing interface. Critical 
mass cannot be reached by limitting the targeted audience to few networking 
experts. However, ease of use involves many elements beyond just expertise. 
Code size is a common albeit subjective metric used to compare programing 
...
differences between the 7 supported communication APIs (excluding self and 
...
is good but too much choice is worse. Fortunately, software programers are 
...
developped in the last two decades: OS-bypass, zero-copy, one-sided and asynchronous operations.
...
in the kernel is often scheduled on a different core when awaken. Avoiding the 
operating system can greatlty improves NUMA locality. 
...
functionality is commonly used in Ethernet adapters supporting Receive Side 
Scaling (RSS).

\emph{Zero-copy} reduces CPU overhead and increase bandwidth by eliminating 
...
of the data to acess, is provided on the origin side. One-sided operations 
...
overlapped with computation. Interestingly, enough computation could completly 
...
data mouvements without blocking the application thread.

To deliver the best performance, a new communication interface should present
semantics that can efficiently leverage all this techniques as provided by 
...
demand/note{need reference} and dynamic buffer management/note{need reference} 
...
requirement, a new communication interface should aims for constant buffer 
...
large systems. Ignoring them is unacceptable. Unfortunately, this is exactely 
...
Unfortunately, both Sockets and Verbs associate buffers to connection, which 
