...
of which the contents vary depending on the event's type. In addition to returing an
...
\emph{backlog} parameter. The call returns a pointer to a \emph{service}.
...
application payload and its length if the client sent it, and the requested conection
attribute.
...
client gets an OTHER event with a type of CONNECT\_TIMEOUT.
